...
def test_default_pipeline_matches_load_raw_plus_simulation():
    ...
    scorecard_nodes = {node.name for node in pipelines["scorecard"].nodes}
    profit_nodes = {node.name for node in pipelines["profit"].nodes}

    assert default_nodes == load_raw_nodes | simulation_nodes
    assert default_nodes.isdisjoint(behavioral_nodes)
    assert default_nodes.isdisjoint(scorecard_nodes)
    assert default_nodes.isdisjoint(profit_nodes)


def test_scorecard_pipeline_registered_with_five_nodes():
    pipelines = register_pipelines()
    assert len(pipelines["scorecard"].nodes) == 5


def test_profit_pipeline_registered_with_three_nodes():
    pipelines = register_pipelines()
    assert len(pipelines["profit"].nodes) == 3
